      ...
      <div class="bg-white dark:bg-gray-800 shadow-xs sm:rounded-lg ">
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <div class="bg-gray-700 p-5 rounded-2xl my-5">
            <h2 class="text-2xl px-2 py-3">Users</h2>
            <table class="border-collapse border border-gray-400 text-sm">
              <thead>
                <tr>
                  <th class="border border-gray-600 py-2 px-4">Name</th>
                  <th class="border border-gray-600 py-2 px-4">Email</th>
                  <th class="border border-gray-600 py-2 px-4">Role</th>
                  <th class="border border-gray-600 py-2 px-4">Depart</th>
                  <th class="border border-gray-600 py-2 px-4">City</th>
                  <th class="border border-gray-600 py-2 px-4">Active</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div class="bg-gray-700 p-5 rounded-2xl my-5">
            <h2 class="text-2xl px-2 py-3">Schedules</h2>
            @if (!empty($allSchedules))
              @foreach ($allSchedules as $index => $depart)
                <div class="border border-gray-400 px-4 py-2">
                  <p>{{ $index }}</p>
                  <ul>
          ...
          <div class="bg-gray-700 p-5 rounded-2xl my-5">
            <h2 class="text-2xl px-2 py-3">Visitors by Resources</h2>
            <table class="w-full border-collapse border border-gray-700 text-sm">
              <thead>
                <th class="border border-gray-600 py-1 px-2">URL</td>
                <th class="border border-gray-600 py-1 px-2">Кол-во</td>
              </thead>
              @foreach ($visitors['byResources'] as $index => $item)
                <tr>
                  <td class="border border-gray-600 py-1 px-2">{{ $index }}</td>
                  <td class="border border-gray-600 py-1 px-2">{{ $item }}</td>
                </tr>
              @endforeach
            </table>
          </div>
          ...
          <div class="bg-gray-700 p-5 rounded-2xl my-5" x-data="{ show: false }">
            <h2 class="text-2xl px-2 py-3">All Visitors</h2>
            ...
            <div x-show="show" class="w-full overflow-x-scroll">
              <table class="border-collapse border border-gray-400 dark:border-gray-700 text-sm">
                <thead>
                  <tr>
                    <th class="border border-gray-600 px-2">ID</th>
                    <th class="border border-gray-600 px-2">IP</th>
                    <th class="border border-gray-600 px-2">User</th>
                    <th class="border border-gray-600 px-2">User Agent</th>
                ...
                @foreach ($visitors['allRows'] as $index => $visitor)
                  <tr class="odd:bg-gray-700 hover:bg-gray-900 text-gray-300">
                    <td class="border border-gray-600 px-2">{{ $visitor->id }}</td>
                    <td class="border border-gray-600 px-2">{{ $visitor->ip }}</td>
                    <td class="border border-gray-600 px-2">{{ $visitor->user }}</td>
                    <td class="border border-gray-600 px-2">{{ $visitor->userAgent }}</td>
                    <td class="border border-gray-600 px-2">{{ $visitor->url }}</td>
                    <td class="border border-gray-600 px-2">{{ $visitor->created_at }}</td>
                  </tr>
                @endforeach
              </table>
            </div>
